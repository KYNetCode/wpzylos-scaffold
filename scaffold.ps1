    ...
    if (Test-Path $scriptPath) {
        ...
        for ($i = 0; $i -lt $RemainingArgs.Count; $i++) {
            ...
            switch -Regex ($arg) {
                ...
                '^-{1,2}skip-qa$' {
                    $buildArgs.SkipQA = $true
                    continue
                }
                '^-{1,2}skip-scoper$' {
                    $buildArgs.SkipScoper = $true
                    continue
                }
                '^-{1,2}version$' {
                    if ($i + 1 -lt $RemainingArgs.Count) {
                        $i++
                        $buildArgs.Version = [string]$RemainingArgs[$i]
                    }
                    continue
                }
                '^-{1,2}phpstan-memory-limit$' {
                    if ($i + 1 -lt $RemainingArgs.Count) {
                        $i++
                        $buildArgs.PhpStanMemoryLimit = [string]$RemainingArgs[$i]
                    }
                    continue
                }
                '^-{1,2}integrity-update-url$' {
                    if ($i + 1 -lt $RemainingArgs.Count) {
                        $i++
                        $buildArgs.IntegrityUpdateUrl = [string]$RemainingArgs[$i]
                    }
                    continue
                }
                '^-{1,2}integrity-update-token$' {
                    if ($i + 1 -lt $RemainingArgs.Count) {
                        $i++
                        $buildArgs.IntegrityUpdateToken = [string]$RemainingArgs[$i]
                    }
                    continue
                }
                default {
        ...
        & $scriptPath @buildArgs @passthrough
    }
    else {
